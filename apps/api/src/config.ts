import "dotenv/config";
import { z } from "zod";

const envSchema = z.object({
  PORT: z.coerce.number().default(4000),
  DATABASE_URL: z.string().min(1, "DATABASE_URL is required"),
  DOCUMENT_STORAGE_DIR: z.string().default("./storage/documents"),
  PUBLIC_API_BASE_URL: z.string().default("http://localhost:4000"),
  QUOTATION_TEMPLATE_DIR: z.string().default("../../templates/quotation-template.xlsx"),
  AIRTABLE_TOKEN: z.string().min(1, "AIRTABLE_TOKEN is required"),
  AIRTABLE_BASE_ID: z.string().min(1, "AIRTABLE_BASE_ID is required"),
  AIRTABLE_ENQUIRIES_TABLE: z.string().default("Enquiries"),
  AIRTABLE_CUSTOMERS_TABLE: z.string().default("Customers"),
  AIRTABLE_QUOTATIONS_TABLE: z.string().default("Quotations"),
  AIRTABLE_QUOTATION_LINE_ITEMS_TABLE: z.string().default("Quotation Line Items"),
  AIRTABLE_ORDERS_TABLE: z.string().default("Orders"),
  AIRTABLE_ORDER_LINE_ITEMS_TABLE: z.string().default("Order Line Items"),
  AIRTABLE_PRODUCTS_TABLE: z.string().default("Products"),
  AIRTABLE_USERS_TABLE: z.string().default("ReshamSutra Users"),
  AIRTABLE_CUSTOMER_CHANGE_LOG_TABLE: z.string().default("Customer Change Log"),
  AIRTABLE_QUOTATION_CHANGE_LOG_TABLE: z.string().default("Quotation Change Log"),
  AIRTABLE_ORDER_CHANGE_LOG_TABLE: z.string().default("Order Change Log"),
  AIRTABLE_PRODUCT_CHANGE_LOG_TABLE: z.string().default("Product Change Log"),
  GOOGLE_DRIVE_ACCESS_TOKEN: z.string().optional().default(""),
  GOOGLE_DRIVE_CLIENTID: z.string().optional().default(""),
  GOOGLE_DRIVE_CLIENTSECRET: z.string().optional().default(""),
  GOOGLE_DRIVE_REFRESH_TOKEN: z.string().optional().default(""),
  GOOGLE_DRIVE_ROOT_FOLDER_ID: z.string().optional().default(""),
  GOOGLE_DRIVE_DEFAULT_TEMPLATE_FOLDER_NAME: z.string().default("Default Templates"),
  GOOGLE_DRIVE_DEFAULT_TEMPLATE_FILE_NAME: z.string().default("quotation-template"),
  ZOHO_BIGIN_CLIENT_ID: z.string().optional().default(""),
  ZOHO_BIGIN_CLIENT_SECRET: z.string().optional().default(""),
  ZOHO_BIGIN_REFRESH_TOKEN: z.string().optional().default(""),
  ZOHO_BIGIN_ACCOUNTS_DOMAIN: z.string().default("https://accounts.zoho.in"),
  ZOHO_BIGIN_API_DOMAIN: z.string().default("https://www.zohoapis.in"),
  ZOHO_BIGIN_REDIRECT_URI: z.string().default("http://localhost:4000/api/integrations/zoho-bigin/callback"),
  ZOHO_BIGIN_SCOPE: z.string().default("ZohoBigin.modules.ALL,ZohoBigin.settings.ALL,offline_access"),
  ZOHO_BIGIN_MODULE_API_NAME: z.string().default("Contacts"),
  ZOHO_BIGIN_DUPLICATE_CHECK_FIELDS: z.string().default("Email,Mobile"),
  ZOHO_BIGIN_COMPANY_FIELD_API_NAME: z.string().default("Account_Name"),
  ZOHO_BIGIN_ACCOUNT_MODULE_API_NAME: z.string().default("Accounts"),
  ZOHO_BIGIN_ACCOUNT_NAME_FIELD_API_NAME: z.string().default("Account_Name"),
  ZOHO_BIGIN_PHONE_FIELD_API_NAME: z.string().default("Phone"),
  ZOHO_BIGIN_DEAL_MODULE_API_NAME: z.string().default("Pipelines"),
  ZOHO_BIGIN_DEAL_DUPLICATE_CHECK_FIELDS: z.string().default("Deal_Name"),
  ZOHO_BIGIN_DEAL_NAME_FIELD_API_NAME: z.string().default("Deal_Name"),
  ZOHO_BIGIN_DEAL_STAGE_FIELD_API_NAME: z.string().default("Stage"),
  ZOHO_BIGIN_DEAL_STAGE_ENQUIRY_VALUE: z.string().default("Enquiry"),
  ZOHO_BIGIN_DEAL_STAGE_TO_QUOTE_VALUE: z.string().default("To Quote"),
  ZOHO_BIGIN_DEAL_STAGE_QUOTED_VALUE: z.string().default("Quoted"),
  ZOHO_BIGIN_DEAL_PIPELINE_ID_FIELD_API_NAME: z.string().default("Pipeline"),
  ZOHO_BIGIN_DEAL_PIPELINE_ID_VALUE: z.string().default("4700291000000006787"),
  ZOHO_BIGIN_DEAL_PIPELINE_FIELD_API_NAME: z.string().default("Sub_Pipeline"),
  ZOHO_BIGIN_DEAL_PIPELINE_NAME: z.string().default("Standard"),
  ZOHO_BIGIN_DEAL_CLOSING_DATE_FIELD_API_NAME: z.string().default("Closing_Date"),
  ZOHO_BIGIN_DEAL_CONTACT_FIELD_API_NAME: z.string().default("Contact_Name"),
  ZOHO_BIGIN_DEAL_ACCOUNT_FIELD_API_NAME: z.string().default("Account_Name"),
  ZOHO_BIGIN_DEAL_PRODUCT_FIELD_API_NAME: z.string().default("Products"),
  ZOHO_BIGIN_DEAL_QUOTATION_REF_FIELD_API_NAME: z.string().default("Quote_Ref"),
  ZOHO_BIGIN_PRODUCT_MODULE_API_NAME: z.string().default("Products"),
  ZOHO_BIGIN_PRODUCT_NAME_FIELD_API_NAME: z.string().default("Product_Name"),
  ZOHO_BIGIN_PRODUCT_CODE_FIELD_API_NAME: z.string().default("Product_Code"),
  ZOHO_BIGIN_DEFAULT_MAILING_COUNTRY: z.string().default("India"),
  AIRTABLE_INTERFACE_URL: z.string().optional().default(""),
  AIRTABLE_ENQUIRY_FORM_URL: z.string().optional().default(""),
  AIRTABLE_LINE_ITEMS_FORM_URL: z.string().optional().default(""),
  AIRTABLE_PRODUCTS_FORM_URL: z.string().optional().default(""),
  META_WHATSAPP_VERIFY_TOKEN: z.string().optional().default(""),
  META_SYSTEM_USER_ACCESS_TOKEN: z.string().optional().default(""),
  META_WHATSAPP_PHONE_NUMBER_ID: z.string().optional().default(""),
  META_WHATSAPP_QUOTATION_TEMPLATE_NAME: z.string().optional().default(""),
  META_WHATSAPP_QUOTATION_TEMPLATE_LANGUAGE: z.string().optional().default("en"),
  META_WHATSAPP_QUOTATION_TEMPLATE_BODY_PARAMS: z.string().optional().default("customerName,filename"),
  PUBLIC_WEB_ORIGIN: z.string().default("http://localhost:3000"),
  SESSION_COOKIE_NAME: z.string().default("resham_sutra_session"),
  SESSION_SECRET: z.string().default("resham-sutra-dev-session-secret-change-me"),
  SALES_TEAM_NOTIFICATION_EMAIL: z.string().optional().default(""),
  SMTP_HOST: z.string().optional().default(""),
  SMTP_PORT: z.coerce.number().default(465),
  SMTP_USER: z.string().optional().default(""),
  SMTP_APP_PASSWORD: z.string().optional().default(""),
  SMTP_FROM_NAME: z.string().optional().default("Resham Sutra Sales"),
  OPENAI_API_KEY: z.string().optional().default(""),
  WHATSAPP_PARSER_OPENAI_MODEL: z.string().optional().default("gpt-4.1-mini"),
  INTAKE_POLLING_ENABLED: z
    .union([z.string(), z.boolean()])
    .transform((value) => String(value).toLowerCase() === "true")
    .default(false),
  INTAKE_POLLING_INTERVAL_MS: z.coerce.number().default(60000)
});

export const env = envSchema.parse(process.env);

